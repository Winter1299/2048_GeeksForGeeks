# logic.py to be imported 
# imported in 2048.py file
...
def start_game():
    ...
    #then appending 4 list each other with four
    #elements as 0
    mat = []
    for i in range(4):
        mat.append([0] * 4)
...
def get_current_state(mat):
    ...
    #else if we have lost the game
    return 'LOST'

# all functions defined below are for left swap
...
def compress(mat):
    ...
    # here we will shift entries of each cell to its extreme
    # left row by row
    ...
    for i in range(4):
        ...
        for j in range(4):
            if(mat[i][j] != 0):
                ...
                # we will shift its number to 
                # prev empty cell in that row
...
def merge(mat):
    ...
    for i in range(4):
        for j in range(3):
            ...
            # if current value has same value as
            # the next cell in the row and they
            # are non empty then
            if(mat[i][j] == mat[i][j + 1] and mat[i][j] != 0):
                ...
                # bool variable true indicating 
                # the new grid after merging is different
...
def move_right(grid):
    ...
    # then again reverse the matrix will
